...
def plugin_func():
  ...
  val = config.getint("Contrast","light")
...
register(
          "python-fu-sumkamnet-contrast-light", # Имя регистрируемой функции
          "sumkam.net. Increases contrast on preconfigured value", # Информация о дополнении
          "Contrast light", # Короткое описание выполняемых скриптом действий
          "Eugene Polyakov", # Информация об авторе
          "Eugene Polyakov (foxen@foxen.ru)", # Информация о копирайте
          "29.12.2013", # Дата изготовления
          "contrast light", # Название пункта меню, с помощью которого дополнение будет запускаться
